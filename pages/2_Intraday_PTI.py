...
st.set_page_config(page_title="Intraday", page_icon="📈")

st.title("Intraday Strategy ")

st.sidebar.subheader('Date parameters')
start_date = st.sidebar.date_input("Start date", datetime.date(2023, 2, 10))
...
def intraday_trade(item,startdate):
    all_d=pd.DataFrame()
    all_mean=pd.DataFrame()
    all_desv=pd.DataFrame()
    closeprice=pd.DataFrame()
    for i in item:
        data=yf.download(i,startdate,interval='5m')
        datacl=data['Close']
...
allof=intraday_trade(ticker,start_date)

st.write(allof[0])

for w in ticker:
    fig=make_subplots(rows=2, cols=1,subplot_titles=(w, "mean/desv", "Desv_10"))
    fig.append_trace(go.Scatter(x=allof[1][w][78:].index,y=allof[1][w][78:]), row=1, col=1)
    fig.append_trace(go.Scatter(x=allof[2][w][68:].index,y=allof[2][w][68:]), row=2, col=1)
...
st.sidebar.header("Intraday Strategy")
